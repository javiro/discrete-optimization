...
def calculate_cost(vehicle_count, vehicle_tours, depot):
...
def reverse_segment_if_better(tour, j, k):
    ...
    d0 = length(C, D) + length(E, F)
    d2 = length(C, E) + length(D, F)
...
def two_opt(tour):
    ...
    while True:
        ...
        for (b, c) in all_segments(len(tour)):
            delta += reverse_segment_if_better(tour, b, c)
        if delta >= 0:
            break
    return tour
...
def optimize_tours(vehicle_count, vehicle_tours, customers):
    optimized_tours = []
    for v in range(0, vehicle_count):
        solution = vehicle_tours[v]
        solution = two_opt(solution)
        optimized_tours.append(solution)
...
def solve_it(input_data):
    ...
    print(vehicle_tours)
    print(depot)
    obj = calculate_cost(vehicle_count, vehicle_tours, depot)
    print("Primer intento: {}".format(obj))

    vehicle_tours = optimize_tours(vehicle_count, vehicle_tours, customers)

    obj = calculate_cost(vehicle_count, vehicle_tours, depot)
    print("Segundo intento: {}".format(obj))
